...
import com.bfn.client.web.DemoUtil
 ...
 */
fun main(args: Array<String>) = RemoteNodeClient().main(args)

private class RemoteNodeClient {
    companion object {
...
//        }
    ...
    private fun showNodeInformation() {
        val host = "nd-601-154-124.rg-788-479.p2pify.com"
        val port = 10201
        logger.info("\uD83E\uDD6C \uD83E\uDD6C setting up NetworkHostAndPort: at $host:$port")
        val bfnNode1 = NetworkHostAndPort(host = host, port = port)
        val rpcUsername = "mystifying_cray"
        val rpcPassword = "device-grimy-herbs-kelp-cramp-sprout"

        val clientNotary = CordaRPCClient(bfnNode1)
        logger.info("\uD83E\uDD6C \uD83E\uDD6C CordaRPCClient:  ${clientNotary.toString()}  " +
                "\uD83E\uDDA0 created from NetworkHostAndPort: $host:$port")
        val proxyPartyA = clientNotary.start(rpcUsername, rpcPassword).proxy
        getThisNode(proxyPartyA)
        logger.info("\n")
        proxyPartyA.notaryIdentities().forEach() {
            logger.info("\uD83D\uDD35 \uD83D\uDD35 Notary Party: \uD83D\uDD35 ${it.name} \uD83D\uDD35 ")
        }
        logger.info("\n")
        proxyPartyA.registeredFlows().forEach() {
            logger.info("\uD83E\uDDA0 \uD83E\uDDA0  Registered Flow: \uD83E\uDDA0 $it \uD83E\uDDA0  ")
        }
        logger.info("\n")
        proxyPartyA.networkMapSnapshot().forEach() {
            logger.info("\uD83D\uDD06 \uD83D\uDD06 Node: \uD83E\uDDA0 ${it.legalIdentities[0]} \uD83D\uDD06 \uD83D\uDD06")
        }
        ...
        try {
            ...
            logger.warn("Account may already exist on the node", e)
        }
        getNodeContractStates(proxyPartyA)
    ...
    private fun createAccountInfo(proxyPartyA: CordaRPCOps) : AccountInfo? {
        ...
        try {
            ...
            val accountInfoCordaFuture = proxyPartyA.startTrackedFlowDynamic(
                    CreateAccountFlow::class.java, name).returnValue
            cnt++
    ...
    private fun getNodeContractStates(proxyPartyA: CordaRPCOps) {
        val states = proxyPartyA.vaultQueryByWithPagingSpec(
                contractStateType = ContractState::class.java,
                criteria = QueryCriteria.VaultQueryCriteria(status = Vault.StateStatus.UNCONSUMED),
                paging = PageSpecification(1, 5000)).states
        logger.info("\n\uD83D\uDC65 Total states on node: \uD83D\uDC65 ${proxyPartyA.nodeInfo().legalIdentities[0].name.organisation}: " +
                "\uD83E\uDD6C  ${states.size}  \uD83D\uDC65  ")
        printAccountInfos(states)
        logger.info("\n")
    }

    private fun printAccountInfos(states: List<StateAndRef<ContractState>>) {
        var cnt = 0
        val dump: MutableList<AccountInfo> = mutableListOf()
        states.forEach() {
            if (it.state.data is AccountInfo) {
                dump.add(it.state.data as AccountInfo)
            }
        }
        val sorted = dump.sortedBy { it.name }
        sorted.forEach() {
            cnt++
            logger.info("\uD83D\uDC9C \uD83D\uDC9C \uD83C\uDF3A Account on Node: \uD83D\uDD06 ${it.identifier.id} " +
                    "\uD83D\uDC9C #$cnt \uD83D\uDD06 ${it.name} \uD83D\uDC4C ")
        }
    }
    ...
    private fun getThisNode(proxy: CordaRPCOps) {
        logger.info("\uD83E\uDD6C \uD83E\uDD6C Getting nodeInfo from proxy with networkParameters: " +
                "\n\uD83D\uDD35 \uD83D\uDD35 ${proxy.networkParameters} \uD83D\uDD35 \uD83D\uDD35")
        val me = proxy.nodeInfo();
        logger.info("\uD83E\uDD6C \uD83E\uDD6C I am connected to host: \uD83E\uDD6C ${me.addresses.first()} " +
                "- \uD83C\uDF4A - node: ${me.legalIdentities.first().name.organisation} \n")
    }
